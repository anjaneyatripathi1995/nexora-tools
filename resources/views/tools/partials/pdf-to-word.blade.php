...
<div class="pdf-convert-hero mb-4">
    ...
    <div class="mt-3">
        <button type="button" class="btn btn-outline-secondary" onclick="alert('PDF to Word conversion will be available when the server service is configured.')">or drop PDF here</button>
    </div>
</div>

@push('scripts')
<script>
document.getElementById('pdf2w_file_input').addEventListener('change', function(){
...
document.querySelector('label[for="pdf2w_file_input"]').addEventListener('click', function(){
    document.getElementById('pdf2w_file_input').click();
});
</script>
@endpush
