...
<div class="panel-card">
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <i class="fa-solid fa-wrench me-2 text-muted"></i>
                        {{ \Illuminate\Support\Str::title(str_replace('-', ' ', $u->tool_slug)) }}
                    </td>
                    <td>{{ $u->created_at->format('M j, Y g:i A') }}</td>
                    <td>
                        <a href="{{ route('tools.show', ['slug' => $u->tool_slug]) }}" class="btn btn-sm btn-outline-primary">Open</a>
                    </td>
                </tr>
                @empty
                <tr>
    ...
    @if(method_exists($usages, 'hasPages') && $usages->hasPages())
