    ...
    <div class="col-lg-6">
        <div class="panel-card h-100">
            ...
            <div class="row g-2">
                ...
                <div class="col-12">
                    <a href="{{ route('tools.show', ['slug' => $item->item_slug]) }}" class="d-flex align-items-center p-2 rounded-2 text-body text-decoration-none panel-nav-link">
                        <i class="fa-solid fa-wrench me-2 text-primary"></i>
                        <span>{{ \Illuminate\Support\Str::title(str_replace('-', ' ', $item->item_slug)) }}</span>
                    </a>
                </div>
                @empty
    ...
    <div class="col-lg-6">
        <div class="panel-card h-100">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <a href="{{ url('/') }}" class="btn btn-sm btn-outline-primary">Explore</a>
            </div>
            <div class="row g-2">
                @forelse($savedProjects ?? [] as $item)
                <div class="col-12">
                    <a href="{{ url('/') }}" class="d-flex align-items-center p-2 rounded-2 text-body text-decoration-none panel-nav-link">
                        <i class="fa-solid fa-folder me-2 text-success"></i>
                        <span>{{ \Illuminate\Support\Str::title(str_replace('-', ' ', $item->item_slug)) }}</span>
                    </a>
                </div>
                @empty
                <p class="text-muted small mb-0">No bookmarks yet.</p>
                @endforelse
            </div>
        </div>
    </div>
...
<div class="panel-card">
    ...
    <div class="list-group list-group-flush">
        ...
        <a href="{{ route('tools.show', ['slug' => $act->tool_slug]) }}" class="list-group-item list-group-item-action border-0 px-0 d-flex align-items-center text-body text-decoration-none">
            <i class="fa-solid fa-wrench text-primary me-3"></i>
            <div class="flex-grow-1">
                <strong>Used {{ \Illuminate\Support\Str::title(str_replace('-', ' ', $act->tool_slug)) }}</strong>
